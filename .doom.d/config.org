...
Now that we've defined my/python-venv-auto-activate, let's run it whenever opening a .py file.
Note that we do not use an "(add-hook 'python-mode-hook ...)" approach here because 'python-mode-hook gets run every time code is sent from the python file to the REPL.
Found from sds's answer [[https://emacs.stackexchange.com/questions/12396/run-command-when-opening-a-file-of-a-specific-filetype][here]].
#+begin_src elisp
